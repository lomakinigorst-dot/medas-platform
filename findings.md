...
## Flash call провайдеры в России (2026-06-15)

**Что такое flash call:** звонок с номера, последние 4 цифры которого = код. Пользователь видит входящий, не отвечает, вводит 4 цифры. Дешевле SMS.

**Провайдеры (по приоритету для MEDAS):**

| Провайдер | Цена flash call | API | Статус |
|---|---|---|---|
| redsms.ru | ~0.45-0.85₽ | HTTP GET/POST | ✅ простой REST |
| websms.ru | уточнить | HTTP | ✅ Flash Call раздел на сайте |
| sigmasms.ru | уточнить | HTTP | ✅ специализированный FlashCall API |
| sms.ru/callcheck | ~0.65₽ | HTTP | ⚠️ УСТАРЕВШИЙ — они сами говорят мигрировать |
| Exolve (МТС) | уточнить | REST | ✅ enterprise, дороже |

**Текущее решение (временное):**
- 3 попытки: `smsc.ru` voice call (диктует 4 цифры), не настоящий flash call
- UX уже говорит "последние 4 цифры входящего номера"
- TODO: заменить `send_flash_call` в `otp.py` на реального провайдера

**Рекомендация:** зарегистрироваться на sigmasms.ru или redsms.ru, получить API ключ, заменить функцию `send_flash_call`.
